...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uLIB
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    // is less than 100000 units:
    if (nMinFee < nBaseFee && mode == GMF_SEND)
    {
        BOOST_FOREACH(const CTxOut& txout, vout)
            if (txout.nValue < 100000)
                nMinFee = nBaseFee;
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x7414249784490e2f8f3afb7dfd08f40cb73e5ff6086f9654329f0a1a58023e80"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe4, 0xdd, 0xe5, 0xda };
