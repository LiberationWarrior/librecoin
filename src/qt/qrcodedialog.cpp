...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("LIB")
    updateDisplayUnit();
}
...
{
    QString ret = QString("librecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LIB unit input in lnReqAmount, we generate the URI with LIB as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LIB, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
